    ...
    return AlertDialog(
      ...
      actions: <Widget>[
        FlatButton(
          child: Text("Yes"),
          onPressed: () {
            BooksDatabaseModel model =
                ScopedModel.of<BooksDatabaseModel>(scaffoldKey.currentContext);
            model
                .uploadBook(book: Book(isbn: isbnCode, title: null, author: null, imagePath: null))
                .then((bool success) {
              if (success) {
                scaffoldKey.currentState.showSnackBar(Consts.SNACKBAR_UPLOAD_SUCCESSFUL);
              } else {
                scaffoldKey.currentState.showSnackBar(Consts.SNACKBAR_UPLOAD_FAILED);
              }
            });
